...
set(allow_missing_toolchain FALSE)
set(expected_missing_toolchain_stderr)
...
if(CASE STREQUAL "valid-default")
    ...
    set(allow_missing_toolchain TRUE)
    set(expected_stdout
    ...
    set(expected_missing_toolchain_stderr
        ...
        "Native link stage unavailable:")
elseif(CASE STREQUAL "valid-emit-obj")
    set(source_file "${CASE_DIR}/valid_simple.vlt")
    if(WIN32)
        set(expected_file "${OUT_DIR}/valid_simple.obj")
    else()
        set(expected_file "${OUT_DIR}/valid_simple.o")
    endif()
    set(args --emit-obj -o "${expected_file}")
    set(allow_missing_toolchain TRUE)
    set(expected_stdout "Emitted object:")
    set(expected_missing_toolchain_stderr "Native toolchain unavailable:")
...
if(NOT "${actual_exit}" STREQUAL "${expected_exit}")
    if(allow_missing_toolchain AND "${actual_exit}" STREQUAL "1")
        set(missing_toolchain_match FALSE)
        foreach(expected IN LISTS expected_missing_toolchain_stderr)
            string(FIND "${actual_stderr}" "${expected}" match_pos)
            if(NOT match_pos EQUAL -1)
                set(missing_toolchain_match TRUE)
            endif()
        endforeach()
        if(missing_toolchain_match)
            message(STATUS "CASE ${CASE}: skipped behavior due to missing native toolchain")
            return()
        endif()
    endif()
    message(FATAL_ERROR
