...
`source -> lexer -> parser -> AST -> semantic analysis -> VIR -> VIR optimize/verify -> backend -> native artifact (target-dependent)`
...
- [Native Output — Current State and Roadmap](#native-output--current-state-and-roadmap)
...
| Backends | Native backend pipeline with production x86_64 targets, experimental aarch64 targets, and LLVM IR text (`--emit-llvm`) |
...
| Backend maturity | Full cross-platform relocation/link coverage, wider ABI coverage, richer target controls |
...
## Native Output — Current State and Roadmap

Voltis currently has one production-native backend: PE/COFF on x86_64. ELF and Mach-O output paths are presently generated by extracting `.text` from the PE payload and wrapping it in minimal platform containers.

That means:

1. Only `.text` is preserved reliably; `.data`, `.rodata`, `.bss`, and richer section layouts are not yet modelled end-to-end.
2. Relocation chains are not fully resolved for the wrapped formats.
3. External import/link resolution is intentionally limited in non-PE wrapper mode.
4. Debug info translation is not implemented for those wrappers.

Roadmap direction:

- Promote dedicated `ELFWriter` / `MachOWriter` pipelines with proper sections, symbols, relocations, and entrypoint semantics.
- Keep AArch64 marked experimental until full codegen + relocation/link integration lands.
- Preserve PE x86_64 as the production baseline while new native writers graduate.
